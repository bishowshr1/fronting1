...
const AdminProfile = () => {
  ...
  const { isLoading, isError, data, error } = useGetAllUsersQuery(userInfo.token);
  const { isLoading: load, isError: err, data: userData, error: msg } = useGetAllOrdersQuery(userInfo.token);
                ...
                <tr key={_id}>
                  ...
                  <td className={classes}>
                    <button onClick={() => nav(`/user/order/${_id}`)}>
                      <Typography as="a" variant="small" color="blue" className="font-normal">
                        Detail..
                      </Typography>
                    </button>
                  </td>
